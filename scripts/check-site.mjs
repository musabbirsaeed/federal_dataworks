...
const uploadStep = workflow.indexOf('uses: actions/upload-pages-artifact@v4');
const deployStep = workflow.indexOf('uses: actions/deploy-pages@v4');

if (!(validateJob > -1 && deployJob > validateJob && uploadStep > deployJob && deployStep > uploadStep)) {
  throw new Error('GitHub Pages workflow must validate first, then upload and deploy from the deploy job.');
}
